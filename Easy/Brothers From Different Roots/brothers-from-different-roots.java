...
class Solution
{
	public static int countPairs(Node root1, Node root2, int x)
	{
		...
		inorder(root1,set1,x);
		inorder(root2,set2,x);
		
		int ans=0;
		for(int i:set1)
		{
		    if(set2.contains(x-i)) ans++;
		}
		
		return ans;
	}
	
	public static void inorder(Node root,Set<Integer> set,int x)
	{
	    if(root==null) return;
	    
	    inorder(root.left, set, x);
	    set.add(root.data);
	    inorder(root.right, set, x);
